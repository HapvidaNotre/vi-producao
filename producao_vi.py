...
if _logo_b64:
    ...
    logo_tag = '<div style="font-size:1.3rem;font-weight:900;color:#1a2a4a;letter-spacing:.1em;text-align:center;margin-bottom:8px">VI LINGERIE</div>'

st.markdown(f"""
<style>
@import url('https://fonts.googleapis.com/css2?family=DM+Sans:wght@300;400;500;600;700&family=DM+Mono:wght@400;500&display=swap');
*,*::before,*::after{{box-sizing:border-box;}}
html,body,[data-testid="stApp"]{{font-family:'DM Sans',sans-serif !important;background:#ffffff !important;color:#1a2a4a !important;min-height:100vh;}}
...
.vi-loading{{position:fixed;inset:0;background:#ffffff;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:9999;animation:vi-fadein .3s ease;}}
...
.vi-loading-text{{font-size:.82rem;font-weight:600;color:#6b7280;letter-spacing:.12em;text-transform:uppercase;animation:vi-pulse 1.4s ease infinite;}}
.vi-section-title{{font-size:.68rem;font-weight:700;color:#4b5563;letter-spacing:.14em;text-transform:uppercase;margin-bottom:16px;display:flex;align-items:center;gap:8px;}}
...
def tela_loading(mensagem="Carregando...", duracao=2.2):
    if _logo_src:
        ...
        img = '<div style="font-size:1.2rem;font-weight:900;color:#1a2a4a;letter-spacing:.1em">VI LINGERIE</div>'
...
def tela_extrato():
    ...
    <div style="text-align:center;margin-bottom:20px">
        {logo_tag}
        <div style="font-size:1.1rem;font-weight:700;color:#1a2a4a;margin-top:4px">Extrato de Produção</div>
        <div style="font-size:.75rem;color:#6b7280;margin-top:2px">Consulta, filtros e download por data e funcionário</div>
    </div>
    """, unsafe_allow_html=True)
...
def tela_operador():
    ...
    if "_operador" not in st.session_state:
        st.markdown(f'<div style="text-align:center;margin-bottom:28px;padding-top:20px">{logo_tag}<div style="font-size:1rem;font-weight:700;color:#1a2a4a;margin-top:6px">Apontamento de Produção</div><div style="font-size:.75rem;color:#6b7280;margin-top:2px">Selecione seu nome para começar</div></div><div class="vi-div"></div>', unsafe_allow_html=True)
    ...
    if "_etapa_idx" not in st.session_state:
        st.markdown(f'<div class="painel-top">{avatar_html(operador,52)}<div style="flex:1"><div style="font-size:1rem;font-weight:700;color:#fff">{operador}</div><div style="font-size:.72rem;color:rgba(255,255,255,.75);margin-top:2px">Selecione a operação</div></div><div style="text-align:right">{logo_tag.replace("margin:0 auto 8px","margin:0")}</div></div>', unsafe_allow_html=True)
        h_turno=fmt_tempo(tempo_turno)
        h_inicio=datetime.fromtimestamp(turno_inicio).strftime("%H:%M")
        st.markdown(f'<div class="painel-resumo-box"><div class="vi-section-title" style="margin-bottom:14px">📊 Resumo do Dia</div><div style="display:grid;grid-template-columns:repeat(3,1fr);gap:12px"><div class="resumo-card"><div class="resumo-label">Pedidos feitos</div><div class="resumo-valor" style="color:#1B5E20">{pedidos_hoje}</div></div><div class="resumo-card"><div class="resumo-label">Hora de início</div><div class="resumo-valor" style="font-size:1.1rem">{h_inicio}</div></div><div class="resumo-card"><div class="resumo-label">Tempo no turno</div><div class="resumo-valor" style="font-size:1rem">{h_turno}</div></div></div>', unsafe_allow_html=True)
        if ultimo_pedido and ultimo_inicio and ultimo_fim:
            dur=ultimo_fim-ultimo_inicio
            st.markdown(f'<div class="ultimo-pedido-box"><div style="font-size:.65rem;color:#991b1b;text-transform:uppercase;letter-spacing:.1em;font-weight:700;margin-bottom:8px">⏱ Último pedido — #{ultimo_pedido}</div><div style="display:flex;gap:24px;flex-wrap:wrap"><div><span style="font-size:.7rem;color:#6b7280">Início: </span><span style="font-family:\'DM Mono\',monospace;color:#1a2a4a;font-size:.85rem">{datetime.fromtimestamp(ultimo_inicio).strftime("%H:%M:%S")}</span></div><div><span style="font-size:.7rem;color:#6b7280">Fim: </span><span style="font-family:\'DM Mono\',monospace;color:#1a2a4a;font-size:.85rem">{datetime.fromtimestamp(ultimo_fim).strftime("%H:%M:%S")}</span></div><div><span style="font-size:.7rem;color:#6b7280">Duração: </span><span style="font-family:\'DM Mono\',monospace;color:#dc2626;font-weight:700;font-size:.85rem">{fmt_tempo(dur)}</span></div></div></div>', unsafe_allow_html=True)
    ...
    if not pedido_atual:
        st.markdown('<div style="font-size:.65rem;color:#6b7280;letter-spacing:.12em;text-transform:uppercase;margin-bottom:10px">Pedido Nº</div>', unsafe_allow_html=True)
        if etapa_idx==0:
        ...
        st.markdown(f'<div style="font-size:.65rem;color:#6b7280;letter-spacing:.12em;text-transform:uppercase;margin-bottom:6px">Pedido em Operação</div><div style="font-family:\'DM Mono\',monospace;font-size:2.8rem;font-weight:700;color:#1a2a4a;letter-spacing:.05em">#{pedido_atual}</div><div style="font-size:.75rem;color:{["#1565C0","#6A0DAD","#1B5E20"][etapa_idx]};margin-top:4px;font-weight:600">{etapa_icon} {etapa_nome}</div><div style="font-size:.7rem;color:#9ca3af;margin-top:8px;font-family:\'DM Mono\',monospace">⏱ {elapsed}</div>', unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)
        st.markdown('<div class="vi-div"></div>', unsafe_allow_html=True)
...
if "_modo" not in st.session_state:
    st.markdown(f'<div style="text-align:center;padding:40px 0 28px">{logo_tag}<div style="font-size:1.05rem;font-weight:700;color:#1a2a4a;margin-top:6px">Sistema de Produção</div><div style="font-size:.75rem;color:#6b7280;margin-top:3px">Vi Lingerie — Linha de Montagem</div></div><div class="vi-div"></div>', unsafe_allow_html=True)
